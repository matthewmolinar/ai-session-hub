...
export function SessionCard({ session, onSignInClick, landing }: SessionCardProps) {
  ...
  const content = (
    ...
    >
      ...
      {/* Facebook-style comments preview */}
      {session.comments.length > 0 && (
        <div className="border-t border-border bg-secondary/30 px-4 py-2.5 space-y-2">
          {session.comments.slice(0, 2).map((comment) => (
            <div key={comment.id} className="flex gap-2">
              ...
              <div className="flex-1 min-w-0">
                <div className="bg-card rounded-lg px-2.5 py-1.5 border border-border inline-block">
                  <span className="text-xs font-semibold text-foreground">@{comment.author}</span>
                  <span className="text-xs text-foreground ml-1.5">{comment.content}</span>
                </div>
          ...
          {session.comments.length > 2 && (
            <button
              onClick={(e) => { e.preventDefault(); e.stopPropagation(); }}
              className="text-xs font-medium text-primary hover:text-primary/80 cursor-pointer pl-8"
            >
